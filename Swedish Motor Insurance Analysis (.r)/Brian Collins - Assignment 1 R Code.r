...
MakeClaimHide9 <- ggplot(insurance_data_excl9, aes(as.factor(Make), Claims))
MakeClaimHide9 + geom_boxplot() + theme_minimal() +ggtitle("Boxplots of Insurance Claims by Vehicle Make") + theme(plot.title=element_text(hjust=0.5)) + labs(x="Vehicle Make", y="Number of Insurance Claims")
...
weighted_payment_residuals <- ggplot(wls_FPM, aes(x=fitted(wls_FPM), y=residuals(wls_FPM)))
...
Payment_Residuals + geom_point(shape=19, alpha=0.5) + geom_hline(yintercept=0, linetype="dashed") + theme_minimal() + ggtitle("Payment Model Residuals Against Fitted Values") + labs(x="Model Predicted Values (Payment ~ Claims + Kilometres + Zone + Bonus)", y="Residuals") + theme(plot.title = element_text(hjust=0.5))
#still unsuccessful at addressing heteroscedasticity

#transforming dependent variable to log for regression modelling, attempted log, log+1, cube, sqrt, Box-Cox and Tukey transformations for dependent variable.

insurance_data_p <- insurance_data
insurance_data_p$Payment <- boxcox(insurance_data_p$Payment ~ 1, lambda = seq(-6, 6, 0.1))
...
write.csv(Coverage_Prediction, "D:/Data Analytics - MSc/Data Analytics & Management/Assignment 1/Exports for Report/Coverage Prediction.csv")
...
write.csv(Bonus_Summary, "./bonussummary.csv")
